...
* `sample_list` (input) - file containing list of samples in the following format:
    ...
    ...
    ```
    In `build` mode, the tool requires compressed (*.gz* / *.fna.gz*) or uncompressed (*.fna*) genome files for each sample (extensions are added automatically). In `build-kmers` mode, corresponding [KMC-generated](https://github.com/refresh-bio/KMC) k-mer files (*.kmc_pre* and *.kmc_suf*) are required. In `build-mh` mode, minhashed k-mer files (*.minhash*) must be generated by `minhash` command prior to the database construction. Note, that minhashing may be done during the database construction by specyfying `-f` option.

Options:
* `-f <filter>` - number from [0,1] interval determining a fraction of all k-mers to be accepted by the minhash filter during database construction (default: 1; applies to `build` and `build-kmers` modes).
...
Parameters:
 * `database` (input) - k-mer database file created by `build` mode,
 * `sample` (input) - sample name; corresponding KMC k-mer files (*.kmc_pre* and *.kmc_suf* ) must exist; minhashing is done automatically, if neccessary,
