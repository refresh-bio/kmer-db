...
Options:
* `-f <filter>` - number from [0,1] interval determining a fraction of all k-mers to be accepted by the minhash filter during database construction (applies to `build` and `build-genomes` modes).
* `-k <kmer_length>` - length of k-mers (apply only to `build-genomes` mode). 
