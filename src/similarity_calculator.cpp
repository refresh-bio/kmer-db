...
// *****************************************************************************************
//
void SimilarityCalculator::one2all_sp(const PrefixKmerDb& db, const kmer_t* kmers, size_t kmersCount, std::vector<std::pair<sample_id_t, num_kmers_t>>& similarities) const {
	// get stuff from database
	const auto& patterns = db.getPatterns();
	int samples_count = db.getSamplesCount();
	const auto& hashtables = db.getHashtables();
	...
	std::unordered_map<sample_id_t, num_kmers_t> samples2counts;
	...
	for (size_t id = lo; id < hi; ++id) {
		...
		switch (num_samples)
		{
		case 3: samples2counts[*p++] += to_add;
		case 2: samples2counts[*p++] += to_add;
		case 1: samples2counts[*p++] += to_add;
		}
	}

	similarities.resize(samples2counts.size());
	i = 0;
	for (const auto& entry : samples2counts) {
		similarities[i++] = entry;
	}
	std::sort(similarities.begin(), similarities.end(), [](const std::pair<sample_id_t, num_kmers_t>& p, const std::pair<sample_id_t, num_kmers_t>& q) {
...
// *****************************************************************************************
//
void SimilarityCalculator::db2db_sp(PrefixKmerDb& db1, PrefixKmerDb& db2, SparseMatrix<uint32_t>& matrix, CBubbleHelper& bubbles) const
{
